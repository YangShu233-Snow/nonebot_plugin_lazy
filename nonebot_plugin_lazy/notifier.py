...
class Notifier:
    ...
    def _build_rollcall_html(items: list[dict]) -> str:
        ...
        body = (
            ...
            f'{"".join(rows)}'
            f"</tbody></table>"
            f'<div class="footer">共 {len(sorted_items)} 条点名'
            f'<div style="margin-top:4px;font-size:11px;color:#b0b0b0;">'
            f'Generated by nonebot_plugin_lazy · LAZY Server</div></div>'
            f"</div>"
        )
    ...
    def _build_todo_html(items: list[dict]) -> str:
        ...
        body = (
            ...
            f'{"".join(rows)}'
            f"</tbody></table>"
            f'<div class="footer">共 {len(sorted_items)} 条待办'
            f'<div style="margin-top:4px;font-size:11px;color:#b0b0b0;">'
            f'Generated by nonebot_plugin_lazy · LAZY Server</div></div>'
            f"</div>"
        )
